...
//       array,
//       (Pastvisits) =>
//         Pastvisits.purpose.toLowerCase().indexOf(query.toLowerCase()) !== -1 ||
//         Pastvisits.checkintime.toLowerCase().indexOf(query.toLowerCase()) !== -1 ||
//         Pastvisits.checkouttime.toLowerCase().indexOf(query.toLowerCase()) !== -1
//     );
//   }
...
export default function VisitPage() {
  ...
  const handleSelectAllClick = (event) => {
    if (event.target.checked) {
      const newSelecteds = Pastvisits.data.map((n) => n.visitor_id);
  ...
  const emptyRows = page > 0 ? Math.max(0, (1 + page) * rowsPerPage - Pastvisits.data.length) : 0;

  const filteredVisits = applySortFilter(Pastvisits.data, getComparator(order, orderBy), filterName);
      ...
      <Container>
        <Stack direction="row" alignItems="center" justifyContent="space-between" mb={5}>
          ...
          {/* <Button
            variant="contained"
            onClick={() => {
              navigate('/visitor-magnt-system-frontend/dashboard/visit/past');
            }}
            ...
            <TableContainer sx={{ minWidth: 800 }}>
              <Table>
                <UserListHead
                  ...
                  rowCount={Pastvisits.data.length}
                  ...
                  {filteredVisits.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((row) => {
                    ...
                    return (
                      <TableRow hover key={id} tabIndex={-1} role="checkbox" selected={selectedVisit}>
                        ...
                        <TableCell align="left">{moment(new Date(checkouttime)).format('LLL')}</TableCell>
